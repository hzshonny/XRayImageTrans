...
def miou(path1, path2):
    imgPredict = cv2.imread(path1)
    imgLabel = cv2.imread(path2)
...
def main():
    getmask = Getmask(path='../seg/bottle/no/')
    getmask.mask()
    base_dir1 = '../seg/bottle/no/BtoA(Amask)'
    base_dir2 = '../seg/bottle/no/BtoA(Bmask)'
    files1 = [os.path.join(base_dir1, file) for file in os.listdir(base_dir1)]
    files2 = [os.path.join(base_dir2, file) for file in os.listdir(base_dir2)]
    files1.sort()
    files2.sort()
    sum = 0
    for i in range(len(files1)):
        mio = miou(files1[i], files2[i])
        sum = sum + mio
